...
// #include <pthread.h>    // for use with pthread_setaffinity_np() , pthread_getaffinity_np()
...
// pthread programs should call these instead of the process-based sched_ routines below
...
// for fortran support
//
int setaffinity_for_nvidia_(int *my_rank, int *PPN, int *my_gpu)
{
	int status;
	status= setaffinity_for_nvidia(*my_rank, *PPN, *my_gpu);
	return(status);
}
...
int cpulistaffinity(int my_gpu, int *cpu_cores, int* ncores,  char * bus_info)
{
	...
	if (pci_bus_info == NULL)
	{
		...
		printf("linux kernel version > 2.6.28 required\n\tfound version %s\n",
				myutsname.release);
		exit(-1);
	}
